 ...
 */
class GenericCollectionEndpoint<TEntity, TElementEndpoint : ElementEndpoint<TEntity>> : ETagEndpointBase {
    /**
    ...
    val createAllowed: Boolean?
        get() = this.isMethodAllowed(HttpMethod.POST)
     ...
     * @throws {@link HttpError}: Other non-success status code
     */
    create(entity: TEntity): TElementEndpoint? {
        ...
        val location = response.headers.get(HttpHeader.Location)

        let elementEndpoint: TElementEndpoint
        if (location) {
            // Explicit element endpoint URI from "Location" header
            elementEndpoint = this.elementEndpoint(this, this.join(location))
        } else {
            try {
                // Infer URI from entity ID in response body
                elementEndpoint = this.get(this.serializer.deserialize(response.clone().text()))
            } catch {
                // No element endpoint
                return undefined
            }
        ...
        (elementEndpoint as CachingEndpoint).responseCache = ResponseCache.from(response)
     ...
     * @throws {@link HttpError}: Other non-success status code
     */
    setAll(entities: TEntity[]) { this.putContent(entities) }
