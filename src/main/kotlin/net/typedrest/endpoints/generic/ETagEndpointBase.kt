 ...
 */
class ETagEndpointBase : Endpoint, CachingEndpoint {
    /**
    ...
    protected fun getContent(): string {
        ...
        val response = this.httpClient.send(this.uri, HttpMethod.Get, new Headers(headers))
        if (response.status !== HttpStatusCode.NotModified || !this.responseCache?.content) {
            this.handle(response)
            this.responseCache = ResponseCache.from(response)
        }
     ...
     * @throws {@link HttpError}: Other non-success status code
     */
    protected fun putContent(content: any): Response {
        val headers = Headers()
     ...
     * @throws {@link HttpError}: Other non-success status code
     */
    protected fun deleteContent(): Response {
        val headers = Headers()
        if (this.responseCache?.eTag) headers.append(HttpHeader.IfMatch, this.responseCache.eTag)
